...
const validPayload = (source, data) => {
  if (typeof data === 'string') {
    ...
    else if ([BCP_SRC_NONE, BCP_SRC_URL, BCP_SRC_IPFS, BCP_SRC_BFP].includes(source)) return Buffer.from(data)
  }
  return data
}
...
class BCP {
  ...
  parse(payload) {
    ...
    if (this.payloadSource === BCP_SRC_URL) {
      PARSE_CHECK(this.chunks.length < 4, 'wrong number of chunks')
      PARSE_CHECK(!checkValidURL(sourceDataBuf.toString()), 'invalid URL')
      this.payloadData = { url: sourceDataBuf }
    }
  ...
  createFromString(str) {
    ...
    else if (checkValidURL(str)) source = BCP_SRC_URL
